...
export default function HomePage() {
  ...
  return (
    <main className="px-3 sm:px-4 md:px-6 lg:px-8 pb-8">
      <h1 className="text-2xl sm:text-3xl md:text-4xl pt-2 pb-1">To Be Read</h1>
      <div>
        <p className="text-sm sm:text-base text-foreground/90 pb-4">Books on your reading list.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
          {books.filter((book) => book.status === "to-be-read").map((book) => (
            <BookCard
              key={book._id}
              _id={String(book._id)}
              title={book.title}
